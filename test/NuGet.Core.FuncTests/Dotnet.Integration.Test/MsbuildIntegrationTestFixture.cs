    ...
    public class MsbuildIntegrationTestFixture : IDisposable
    {
        ...
        private void CopyRestoreArtifacts(string artifactsDirectory, string pathToSdkInCli, string configuration, string toolsetVersion)
        {
            ...
            foreach (var projectName in sdkDependencies)
            {
                ...
                IEnumerable<DirectoryInfo> frameworkArtifactFolders = projectArtifactsFolder.EnumerateDirectories().Where(folder => folder.FullName.Contains("net5.0"));
        ...
        private void CopyPackSdkArtifacts(string artifactsDirectory, string pathToSdkInCli, string configuration, string toolsetVersion)
        {
            ...
            // Copy the pack SDK.
            //Order by fullname so that we can get the latest net5.0 version. 
            var packProjectCoreArtifactsDirectory = new DirectoryInfo(Path.Combine(artifactsDirectory, packProjectName, toolsetVersion, "bin", configuration)).EnumerateDirectories("net5.0").OrderBy(x => x.FullName).Last();
        ...
        private void PatchDepsJsonWithNewlyAddedDlls(string[] assemblyNames, string[] filePaths)
        {
            foreach (string assemblyName in assemblyNames)
            {
                foreach (string filePath in filePaths)
                {
                    ...
                    var jproperty = new JProperty("lib/net5.0/" + assemblyName,
                        new JObject
                        {
